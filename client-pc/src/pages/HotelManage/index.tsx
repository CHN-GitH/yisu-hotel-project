...
import { getHotelList, publishHotel, offlineHotel, pauseHotel, resumeHotel, deleteHotel, auditHotel } from '@/api/hotel'
...
const statusMap: Record<string, { color: string; text: string }> = {
  ...
  paused: { color: 'warning', text: '暂停营业' },
...
function HotelManage() {
  ...
  const handleResume = async (id: string) => {
    try {
      await resumeHotel(id)
      message.success('已恢复营业')
      fetchData()
    } catch (error) {
        ...
        <Space size="small">
          ...
          {!isAdmin && (
            <>
              ...
              {((record.status === 'offline' || record.status === 'draft') && !record.pendingAction) && (
                <Popconfirm
                  title="确认发布？"
              ...
              {(record.status === 'online') && !record.pendingAction && (
                <>
                  <Popconfirm
                    title="确认下线？"
                  ...
                  >
                    <Button type="text" icon={<CloseOutlined />} style={{ color: '#fa8c16' }}>
                      暂停营业
                    </Button>
                  </Popconfirm>
                </>
